...
@section('content')
  <div class="card shadow mb-4 col-8">
    ...
    <div class="card-body ">
      ...
      <form action="/perusahaan/{{ $perusahaan->id }}" method="POST" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="row mb-3">
          <label for="inputText" class="col-sm-2 col-form-label">nama</label>
          <div class="col-sm-10">
            <input type="text" name="name" class="form-control" value="{{ $perusahaan->name }}">
          </div>
        </div>
        <div class="row mb-3">
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <input type="text" name="no_fax" class="form-control" value="{{ $perusahaan->no_fax }}">
          </div>
        </div>
        <div class="row mb-3">
          <label for="inputText" class="col-sm-2 col-form-label">tahun</label>
          <div class="col-sm-10">
            <input type="text" name="tahun" class="form-control" value="{{ $perusahaan->tahun }}">
          </div>
        </div>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <select class="form-select" name="pengiriman" aria-label="Default select example">
              <option selected>{{ $perusahaan->pengiriman }}</option>
          ...
          <div class="col-sm-10">
            <select class="form-select" name="kelurahan_id" aria-label="Default select example">
              ...
              @foreach ($kelurahan as $kelu)
                <option value="{{ $kelu->id }}" {{ $kelu->id == $perusahaan->kelurahan_id ? 'selected' : '' }}>
                  {{ $kelu->name }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div class="col-sm-10">
            <select class="form-select" name="kecamatan_id" aria-label="Default select example">
              ...
              @foreach ($kecamatan as $keca)
                <option value="{{ $keca->id }}" {{ $keca->id == $perusahaan->kecamatan_id ? 'selected' : '' }}>
                  {{ $keca->name }}</option>
              @endforeach
            </select>
          </div>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <select class="form-select" name="status_id" aria-label="Default select example">
              <option selected>-- pilih menu --</option>
              @foreach ($status as $s)
                <option value="{{ $s->id }}" {{ $s->id == $perusahaan->status_id ? 'selected' : '' }}>
                  {{ $s->name }}</option>
              @endforeach
            </select>
          </div>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <select class="form-select" name="jenis_barang_id" aria-label="Default select example">
              <option selected>-- pilih menu --</option>
              @foreach ($jenis as $j)
                <option value="{{ $j->id }}" {{ $j->id == $perusahaan->jenis_barang_id ? 'selected' : '' }}>
                  {{ $j->name }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div class="col-sm-10">
            <select class="form-select" name="layanan_id" aria-label="Default select example">
              ...
              @foreach ($layanan as $l)
                <option value="{{ $l->id }}" {{ $l->id == $perusahaan->layanan_id ? 'selected' : '' }}>
                  {{ $l->hari_operasional }}</option>
              @endforeach
            </select>
          </div>
        </div>
